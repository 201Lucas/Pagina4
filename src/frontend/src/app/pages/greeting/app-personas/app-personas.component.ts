  ...
  async addPerson() {
    ...
    `,
      ...
      if (persona.status >= 200 && persona.status <= 205) {
        Swal.fire({
          title: "Creado",
          text: `${nombreInput.value} se a creado el cliente`,
          icon: "success",
        }).then((ok) => {
          if (ok.value) {
            this.getPeople();
        ...
        Swal.fire({
          title: "Error",
          text: `No se pudo eliminar a ${nombreInput.value}`,
          icon: "error",
...
async UpdatePerson(person: any) {
  ...
  `,
    ...
    if (persona.status >= 200 && persona.status <= 205) {
      Swal.fire({
        title: "Editado",
        text: `${nombreInput.value} se a editado el cliente`,
        icon: "success",
      }).then((ok) => {
        if (ok.value) {
          this.getPeople();
      ...
      Swal.fire({
        title: "Error",
        text: `No se pudo editar a ${nombreInput.value}`,
        icon: "error",
